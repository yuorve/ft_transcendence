...
async function gamesRoutes(fastify) {
  ...
  // Obtener TODOS los torneos (y sus partidas) en los que participa un usuario
  fastify.get('/mytournaments/:username', async (request, reply) => {
    try {
      ...
      ORDER BY t.round ASC, g.game_order ASC
    `, [username, username]);
      ...
      // Agrupamos filas por tournament_id
      const mapTournaments = {};
      for (const r of rows) {
        if (!mapTournaments[r.tournament_id]) {
          mapTournaments[r.tournament_id] = {
            tournament: r.tournament_id,
            champion: r.champion,
            created_at: r.tournament_created_at,
            games: []
          };
        }
        mapTournaments[r.tournament_id].games.push({
          game: r.game,
          type: r.type,
      ...
      // Convertimos el mapa en array
      const tournaments = Object.values(mapTournaments);
  ...
  // Obtener torneo completo por ID
  fastify.get('/tournament/:tournamentId', async (request, reply) => {
    ...
    `, [tournamentId]);
      ...
      // Desempaquetamos datos generales
      const { tournament_id, champion, tournament_created_at } = rows[0];

      // Construimos el array de partidas
