...
function inviteToGame(targetUsername: string) {
	...
	// Almacenamos la invitación localmente para seguimiento
	const localInvitations = ref<{ to: string; gameId: string }[]>([]);
	localInvitations.value.push({
		to: targetUsername,
		gameId: gameId
	});
}
...
function respondToGameInvite(to: string, gameId: string, accepted: boolean) {
	...
	// Si aceptó, redireccionamos directamente al juego
	if (accepted) {
		router.push({
			path: '/pong-online',
			query: {
...
<template>
	<div>
		...
		<button v-if="isAuthenticated" v-show="!isOpen" @click="toggleChat"
		...
		<div v-if="isAuthenticated" id="chatWrapper" class="relative w-full h-full">
			...
			<div v-show="isOpen" id="liveChatContainer"
				...
				<div
					...
					<span class="text-sm sm:text-base md:text-lg lg:text-xl">Live Chat</span>
				...
				<div class="flex flex-col md:flex-row flex-1 h-full overflow-hidden">
					<!-- Lista de Usuarios -->
					<div id="userListContainer" :class="{
						hidden: !showUserList && !isDesktop,
						'flex flex-col': showUserList && !isDesktop,
						block: true,
						'md:w-1/3 lg:w-1/3': isDesktop,
						'w-full': !isDesktop,
					}" class="bg-white p-2 md:p-3 lg:p-4 border-b md:border-b-0 md:border-r border-gray-300 overflow-y-auto" :style="{
						...
						<ul id="userList" class="space-y-1 md:space-y-1.5 w-full text-xs sm:text-sm">
							<li v-for="(player, id) in players" :key="id" :class="blockedUsers.includes(player.username)
								...
								<!-- Tooltip -->
								<div v-show="selectedUser === player.username"
									...
									<img :src="profileImages[player.username]" alt="Avatar"
									...
									<button v-if="
										player.username !== auth?.username
									" class="w-full px-2 py-1 rounded text-left text-xs sm:text-sm transition-colors" :class="blockedUsers.includes(
										player.username
									)
										? 'bg-red-200 hover:bg-red-300 text-green-800'
					...
					<button id="toggleUserListButton" @click="toggleUserList"
					...
					<!-- Chat -->
					<div id="chatContainer" class="relative w-full flex-1 flex flex-col overflow-hidden" :style="{
						...
						<div id="chatBox" ref="chatBoxRef"
							...
							<div v-for="(msg, index) in messages" :key="index" class="flex flex-col max-w-full" :class="{
								...
								<div class="rounded-lg px-2 py-1 sm:px-3 sm:py-2 text-xs sm:text-sm md:text-base break-words shadow-sm"
									:class="{
				...
				<!-- Input de mensajes -->
				<div id="chatInputContainer" class="flex p-2 sm:p-3 bg-white border-t border-gray-200">
					<textarea id="chatInput" placeholder="Escribe un mensaje..." v-model="message" @keydown="
						(e) => {
			...
			<!-- Chats minimizados -->
			<div v-show="minimizedChatsVisible.length > 0"
				class="fixed z-50 flex flex-row items-center overflow-x-auto whitespace-nowrap py-1 px-2" :style="{
					bottom: '70px',
					right: '7px',
					width: 'calc(100% - 40px)',
					maxWidth: '600px',
				...
				<div v-for="chat in minimizedChatsVisible.slice().reverse()" :key="`min-${chat.username}`"
					...
					<span class="text-xs font-medium text-black-800 truncate max-w-[80px] sm:max-w-[120px]">{{
						chat.username }}</span>
			...
			<div class="fixed z-50">
				<!-- Chats abiertos -->
				<div v-for="(chat, index) in openChatsVisible" :key="`open-${chat.username}`"
					class="bg-white border border-gray-200 rounded-lg shadow-lg overflow-hidden flex flex-col" :style="{
						position: 'fixed',
						bottom: '60px',
					...
					<div class="bg-blue-500 text-white p-2 flex justify-between items-center">
						<span class="font-medium text-sm truncate max-w-[150px]">{{ chat.username }}</span>
						<div class="flex items-center space-x-1">
							<button @click="minimizeChat(chat.username)"
							...
							<button @click="closePrivateChat(chat.username)"
								class="p-1 hover:bg-blue-600 rounded transition-colors">
								<svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
									<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
					...
					<div class="flex flex-col flex-1 overflow-hidden">
						<!-- Mensajes -->
						<div :id="`private-chat-messages-${chat.username}`"
							class="flex-1 overflow-y-auto p-2 space-y-2">
							<div v-for="(msg, idx) in chat.messages" :key="idx" :class="[
								msg.from === auth?.username
									? 'flex justify-end'
									: 'flex justify-start',
								'w-full message-item',
							...
							<div v-for="invite in incomingInvites" :key="invite.gameId"
								...
								<div class="flex gap-2 mt-2">
									<router-link
										...
										}" @click="respondToGameInvite(invite.from, invite.gameId, true)">
										Aceptar
									</router-link>
									<button class="bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded"
										@click="respondToGameInvite(invite.from, invite.gameId, false)">
										Rechazar
									</button>
								</div>
							</div>
						...
						<div class="p-2 border-t border-gray-200 flex-shrink-0">
							...
							">
								<div class="flex w-full">
									<textarea :id="`privateChatInput-${chat.username}`" placeholder="Mensaje..."
										rows="1"
