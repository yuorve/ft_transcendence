...
const incomingTournamentInvites = ref<{ tournamentId: string; from: string }[]>(
    []
);

// 2. Agregar esta función para responder a invitaciones de torneo

const respondToTournamentInvite = (
    from: string,
    tournamentId: string,
    accepted: boolean
) => {
    ...
    incomingTournamentInvites.value = incomingTournamentInvites.value.filter(
        (invite) => invite.tournamentId !== tournamentId
    );
};
    ...
    websocketState.messages.forEach((data, index) => {
        if (index >= websocketState.processedMessages) {
            ...
            // Actualizar lista de jugadores
                ...
                updatePlayersList();
            } else if (data.type === "tournament-invite") {
                // Agregar la invitación al array de invitaciones de torneo
                incomingTournamentInvites.value.push({
                    from: data.creator,
                ...
                const inviteMessage = {
                    from,
                    message: `Te ha invitado a un torneo de ${data.gameType} (${data.totalPlayers} jugadores)`,
                    timestamp: undefined,
                    openChat: false,
                    tournamentId: data.tournamentId,
                    tournamentInviteButtons: true,
                    gameType: data.gameType,
                    totalPlayers: data.totalPlayers,
                };

                // Si el chat ya existe, solo agregar el mensaje
                if (chatIndex >= 0) {
                    privateChats.value[chatIndex].messages.push(inviteMessage);
                } else {
                    // Si el chat no existe, creamos uno nuevo
            ...
            } else if (data.type === "tournament-invite-response") {
                const from = data.from;
                const accepted = data.accepted;
                const responseMessage = accepted
                    ? `Acepto tu invitación al torneo. ¡Que comience!`
                    : `Rechazo tu invitación al torneo.`;
                ...
                if (accepted) isOpen.value = !isOpen.value;
                // Buscar o crear el chat privado
                const chatIndex = privateChats.value.findIndex(
                    (c) => c.username === from
                );
                ...
                >
                    ...
                    <div class="flex flex-col flex-1 overflow-hidden">
                        ...
                        >
                            ...
                            <div
                                v-for="tournamentInvite in incomingTournamentInvites"
                                :key="tournamentInvite.tournamentId"
                                class="bg-purple-100 p-2 rounded shadow mb-2"
                            >
                                <p
                                    class="text-purple-900 text-sm font-semibold"
                                >
                                    Invitación a Torneo de
                                    {{ tournamentInvite.gameType }}
                                </p>
                                <p class="text-purple-700 text-xs">
                                    {{ tournamentInvite.totalPlayers }}
                                    jugadores totales
                                </p>
                                <div class="flex gap-2 mt-2">
                                    <button
