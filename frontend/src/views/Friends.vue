...
import { actionRequest, getFriends, getRequests, getUsers } from '../api'
...
const rejectRequest = async (id: string) => {
	await actionRequest(id, '1', '0'); // marca como rechazada
	await loadRequests();
};
...
<template>
		...
		<table class="table-auto w-full">
			...
			<tbody>
				<tr v-for="sol in requests" :key="sol.id">
					<td class="border px-4 py-2">{{ sol.username }}</td>
    ...
    <table class="table-auto w-full border bg-red-400">
      <thead>
      ...
      <tbody>
        <tr v-for="friend in friends" :key="friend.id">
          ...
          <td class="border px-4 py-2">
            ...
            <router-link
              :to="{ name: 'Chats', params: { buddy: friend.buddy } }"
