...
const gameMode = ref(route.query.mode || "newGame");
const playerRole = ref(route.query.player || ""); // Will be set from socket message
...
const paddlesInitialized = ref(false);
...
// Function to correctly set up paddle controls based on player role
function initializePaddles() {
  if (paddlesInitialized.value) return;
  
  console.log("Initializing paddles with mode:", gameMode.value, "player role:", playerRole.value);
  
  // If we're coming from an invitation flow, use the playerRole
  if (playerRole.value === "player1") {
    ...
    console.log("Player 1 (host) paddles initialized");
  } 
  else if (playerRole.value === "player2") {
    puntuation.isHost = false;
    puntuation.playerPaddle = scene?.getMeshByName("paddle2");
    puntuation.opponentPaddle = scene?.getMeshByName("paddle1");
    console.log("Player 2 paddles initialized");
  }
  // For the regular flow (direct game creation)
  else if (gameMode.value === "newGame") {
    puntuation.isHost = true;
    puntuation.playerPaddle = scene?.getMeshByName("paddle1");
    puntuation.opponentPaddle = scene?.getMeshByName("paddle2");
    console.log("New game creator paddles initialized");
  } 
  else {
    puntuation.isHost = false;
    puntuation.playerPaddle = scene?.getMeshByName("paddle2");
...
if (socket) {
    // Only send this if we're not coming from an invitation
    ...
    socket.addEventListener('message', event => {
        ...
        if (data.type === 'startGame') {
          ...
          // If we're coming from an invitation, set the player role
          if (data.player) {
            playerRole.value = data.player;
            console.log("Player role set to:", data.player);
          }
          
          // Initialize paddles with the correct settings
          initializePaddles();

          // Set up ball sync if host
          ...
          if (gameMode.value === 'newGame') {
              player1.value = auth?.username || 'ErrorUser';
              player2.value = data.id;                
              console.log("Assigned Player 2:", data.id);
              
              // Initialize paddles if not already done
              if (!paddlesInitialized.value) {
                puntuation.isHost = true;
                puntuation.playerPaddle = scene?.getMeshByName("paddle1");
                puntuation.opponentPaddle = scene?.getMeshByName("paddle2");
                paddlesInitialized.value = true;
              ...
              console.log("Assigned Player 1:", data.id);
          ...
          if (!puntuation.isHost || puntuation.gameState === 'waiting') {
          ...
          puntuation.pr = data.score2;
            ...
            console.log("Opponent moved to:", data.x);
            if (puntuation.opponentPaddle) {
              puntuation.opponentPaddle.position.y = data.x;
            } else {
              console.warn("Opponent paddle not initialized yet!");
            }
        } 
...
onMounted(() => {
  try {
    puntuation.gameMode = gameMode.value;
    ...
    // If we already have player roles from the invitation process, 
    // initialize paddles right away
    if (playerRole.value) {
      setTimeout(() => {
        initializePaddles();
      }, 500); // Short delay to ensure scene is fully loaded
    }
    
    console.log("Pong initialized with:", {
...
watch(
  () => puntuation.pl,
  (newVal) => {
    console.log("punto izquierdo:", newVal);
    if (socket) {
...
watch(
  () => puntuation.pr,
  (newVal) => {
    console.log("punto derecho:", newVal);
    if (socket) {
    ...
    if (newVal >= 5) {
      ...
      sendPunt(player2.value);
    }
  }
);

// Debug logging for paddle movement
watch(
  () => puntuation.playerPaddle?.position.y,
  (newVal) => {
    if (newVal !== undefined) {
      console.log("Player paddle moved to:", newVal);
      if (socket) {
...
watch(
  ...
  ([newDx, newDy]) => {    
    const ball = scene?.getMeshByName("mysphere");
    if (ball && socket && puntuation.isHost) {
      socket.send(JSON.stringify({ 
        type: 'ballUpdate', 
        gameId: gameid, 
        ball: {
