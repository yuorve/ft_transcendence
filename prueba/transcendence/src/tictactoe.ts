...
public static CreateScene(engine: BABYLON.Engine, canvas: HTMLCanvasElement): BABYLON.Scene {
  const scene = new BABYLON.Scene(engine);
        ...
        function createCircle(positionX, positionY, positionZ, scale) {
        ...
        function createX(positionX, positionY, positionZ, scale) {
        ...
        function createFigure(positionX, positionY, positionZ) {
            if (figure == 1) {
        ...
        for (let y = 0; y < 3; y++) {
            buttons[y] = [];
            for (let x = 0; x < 3; x++) {
                let button = new BABYLON.GUI.HolographicButton(`button${y},${x}`);
                buttons[y][x] = button;
            }
        }
        function createButton(y, x, posY, posX) {
            ...
            for (let x = 0; x < 3; x++) {
                buttons[i][x].onPointerUpObservable.add(() => {
                    ...
                    if (checkVic() == true && figure == 2) {
                        ...
                        createCircle(0, 0, -3, butSize * 1.2);
                    }
                });
            }
        }
