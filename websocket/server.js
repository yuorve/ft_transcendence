    ...
    ws.on("message", (message) => {
        try {
            ...
            if (data.type === "game-invite") {
              ...
              const isBlocked = userBlocks[toUsername] && userBlocks[toUsername].includes(fromUsername);
              
              // Verificar si el remitente ha bloqueado al destinatario
              const hasBlockedRecipient = userBlocks[fromUsername] && userBlocks[fromUsername].includes(toUsername);
          
              // Si cualquiera de los dos ha bloqueado al otro, no enviar la invitación
              if (isBlocked || hasBlockedRecipient) {
                  // Opcionalmente, puedes notificar al remitente que la invitación no se pudo enviar
                  ws.send(
                      JSON.stringify({
                          type: "inviteBlocked",
                          message: "No se pudo enviar la invitación. El usuario no está disponible.",
                          targetUser: toUsername
                      })
                  );
                  return; // Salir sin procesar la invitación
              }
            ...
            if (data.type === "game-invite-response") {
                ...
                if (accepted) {
                    ...
                    if (players[toUsername])
                        players[toUsername].gameId = gameId;
                    if (players[fromUsername])
                        players[fromUsername].gameId = gameId;
                    ...
                    broadcast(JSON.stringify({ type: "currentGames", games }));
                    broadcast(
                        JSON.stringify({ type: "currentPlayers", players })
                    );
            ...
            if (data.type === "tournament-invite-response") {
                ...
                console.log(
                    `Respuesta de torneo de ${fromUsername}: ${
            ...
            if (data.type === "globalChat") {
                ...
                wss.clients.forEach((client) => {
                    const senderIsBlocked = userBlocks[
                        client.username
                    ]?.includes(ws.username);
                ...
                if (games[data.id]) {
                    if (!games[data.id].player2) {
                        ...
                        const player1 = clientsArray.find(
                            (client) =>
                                client.username === games[data.id].player1
                        );
                        const player2 = clientsArray.find(
                            (client) =>
                                client.username === games[data.id].player2
                        );
            ...
            if (data.type === "score") {
                ...
                if (games[data.gameId]) {
                    ...
                    if (oponenteId) {
                        ...
                        if (
                            oponente &&
                            oponente.readyState === WebSocket.OPEN
                        ) {
                            oponente.send(
                                JSON.stringify({
                                    type: "scoreUpdate",
            ...
            if (data.type === "opponentMove") {
                ...
                if (
                    ...
                    player1.send(
                        JSON.stringify({ type: "opponentMove", x: data.x })
                    );
